...
開啟網頁
  ...
  │    └─ 失敗 → 自動切換 ↓
  │
  └─ 改讀「快照檔 data.json」→ 🟡 快照資料：顯示存檔時間並標示非即時
```

畫面左上角的徽章會明確標示現在是哪一種模式，快照模式還會跳一條黃色提示帶，
說明資料抓取於什麼時候、要看即時資料請連 VPN——**不會讓人誤把舊資料當成當下行情**。
...
│
└── .github/workflows/snapshot.yml GitHub Actions 排程（每小時）
```
...
- **改完 `app.js` 或 `style.css` 要記得把 `index.html` 裡的 `?v=` 數字加一**，
  否則瀏覽器會一直用快取的舊版本。
  網頁載入時 console 會印出 `[基隆選舉監控] app.js v2`，
